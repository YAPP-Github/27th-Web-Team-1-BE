...
class KakaoLoginServiceTest {
    @Mock
    lateinit var oAuthClientRegistry: OAuthClientRegistryPort
    ...
    fun setUp() {
        kakaoLoginService =
            KakaoLoginService(
                ...
                createCoupleUseCase,
                lockManager =
                    kr.co.lokit.api.common.concurrency
                        .LockManager(),
                cacheManager,
            )
    ...
    fun `탈퇴 복구 가능 기간이 만료된 사용자는 로그인 시 예외가 발생한다`() {
        ...
        whenever(userRepository.findByEmail("test@test.com")).thenReturn(withdrawnExpiredUser)
